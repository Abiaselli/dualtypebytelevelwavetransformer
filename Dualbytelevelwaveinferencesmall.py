...
def byte_sequence_to_text(byte_sequence):
    ...
    """
    decoded_text = ''.join(
        chr(int(byte)) if 0 <= int(byte) <= 256 else '?' for byte in byte_sequence if byte != EOS_TOKEN
    )
    return decoded_text
...
def logits_to_tokens(logits, temperature=1.0, top_k=0, top_p=0.0, max_vocab=256):
...
def tokens_to_byte_sequence(tokens, max_vocab=257):
    """
    ...
    def load_model_and_tokenizer(self, model_path):
        ...
        if architecture == 'matmul_free':
            model = WaveCascadeTransformer(
                ...
                vocab_size=257,  # Byte embeddings cover values 0–255
