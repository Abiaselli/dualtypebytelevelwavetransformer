...
token_length = 40 #must match length of EOS binary
max_seq_len = 1024
...
def visualize_wave(binary_input, embed_size, device):

    wave, probabilities = bytes_to_wave_embeddings(
    ...
    for i in range(min(len(wave), 5)):  # Show first 5 indices
        plt.plot(wave[i], label=f"Index {i}")
    plt.title("Wave Function Visualization")
...
class WaveCascadeTransformer(nn.Module):
    def __init__(self, num_nodes, hidden_size, num_heads, max_seq_length, vocab_size, node_type, num_layers, include_preprocessing_node=True):
        ...
        self.output_layer = nn.Linear(vocab_size, hidden_size)  # Predict binary probabilities for vocab_size binary positions.
        self.device=device
    ...
    def forward(self, input_text, mask=None):
        ...
        wave_embeddings=self.output_layer(wave_embeddings)
    ...
    def test_wave_embeddings(self):
        ...
        if sample_text:
            try:
                ...
                #visualize_wave(preprocess_text(text_batch), embed_size, device)
