...
EOS= "<<EOS>> "
EOS_TOKEN = "00111100 00111100 01000101 01001111 01010011 00111110 00111110 00100000"
EOS_BINARY = 0o0011110000111100010001010100111101010011001111100011111000100000
EOS_BINARY_INT = int("0011110000111100010001010100111101010011001111100011111000100000", 2)  # Converts binary to int
...
def preprocess_text(text, max_seq_len=1024, chunk_size=64):
    binary_sequence = []
    for char in text:
...
def bytes_to_wave_embeddings(byte_sequences, embed_size, device, binary_length=64):
    """
...
def bytes_to_wave_embeddings_single(query_tensor, embed_size, device, binary_length=64):
    """
...
class MiniTransformerNode(nn.Module):
    ...
    def forward(self, x, prev_node_output=None, src_mask=None, is_final_node=False):
        ...
        x = torch.clamp(x.long(), min=0, max=64)
...
class MatMulFreeLanguageModel(nn.Module):
    def __init__(self, embed_size, hidden_size, num_heads, max_seq_length, vocab_size=64, eps=1e-8, device =device):
        super().__init__()
        self.eps = eps
    ...
    def load_model(self):
        try:
            ...
            self.model = WaveCascadeTransformer(
                ...
                max_seq_length=self.max_seq_length.get(),
                num_layers=self.num_layers.get(),
                vocab_size=64,  # Larger vocab size for extended tokens
...
if __name__ == "__main__":
    ...
    mock_queries = [torch.randint(0, 2, (128,)) for _ in range(2)]  # Example binary sequences
    mock_targets = [[1, 0, 1], [1, 1, 0]]  # Example targets
    mock_batch = list(zip(mock_queries, mock_targets))
